...
$Enabled = true;
$strListHead = (new ListTools())->TableHead(array(FA_LC["username"], FA_LC["group_name"]), FA_LC["tools"]);
...
$strListBody = '';
foreach ($objORM->FetchAllWhitoutCondition('UserName,iw_admin_group_id,Enabled,id', TableIWAdmin) as $ListItem) {

    $ListItem->UserName = $objGlobalVar->de2Base64($ListItem->UserName);


    $ListItem->iw_admin_group_id = @$objORM->Fetch(
        "id = $ListItem->iw_admin_group_id",
        'Name',
        TableIWAdminGroup
    )->Name;
    ...
    $strListBody .= (new ListTools())->TableBody($ListItem, $ToolsIcons, 2, $objGlobalVar->en2Base64($ListItem->id . '::==::' . TableIWAdmin, 0));
}