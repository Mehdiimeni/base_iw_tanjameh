...
$Enabled = true;
$strListHead = (new ListTools())->TableHead(array(FA_LC["row"], FA_LC["name"], FA_LC["super_admin"]), FA_LC["tools"]);
...
foreach ($objORM->FetchAllWhitoutCondition('IdKey,Name,SuperAdmin,Enabled,IdRow', TableIWAdminGroup) as $ListItem) {
    ...
    $strListBody .= (new ListTools())->TableBody($ListItem, $ToolsIcons, 3, $objGlobalVar->en2Base64($ListItem->IdKey . '::==::' . TableIWAdminGroup, 0));
}
