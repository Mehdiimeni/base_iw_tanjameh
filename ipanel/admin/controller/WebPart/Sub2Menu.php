...
$strListBody = '';
foreach ($objORM->FetchAllWhitoutCondition('IdKey,Name,LocalName,GroupIdKey,WeightIdKey,Enabled,IdRow', TableIWWebSub2Menu) as $ListItem) {

    $ListItem->LocalName = '<input type="text" class="name-sub2" maxlength="250" size="25" id="' . $ListItem->IdKey . '" value="' . $ListItem->LocalName . '">';
    ...
    $ListItem->WeightIdKey =  '<input type="text" class="weight-sub2" maxlength="3" size="3" id="' . $ListItem->Name . '" value="' . @$objORM->Fetch($SCondition,'Weight',TableIWWebWeightPrice)->Weight . '">';
    ...
    $strListBody .= (new ListTools())->TableBody($ListItem, $ToolsIcons, 5, $objGlobalVar->en2Base64($ListItem->IdKey . '::==::' . TableIWWebSub2Menu, 0));
}
