...
$strMenuIdKey = '';
$SCondition = " Enabled = '$Enabled' ORDER BY IdRow ";
foreach ($objORM->FetchAll($SCondition, 'Name,IdKey', TableIWWebMainMenu) as $ListItem) {
...
if (!isset($_POST['SubmitApi']) and @$objGlobalVar->RefFormGet()[0] != null) {
    ...
    $objEditView = $objORM->Fetch($SCondition, 'Name,LocalName,GroupIdKey,WeightIdKey,Description', TableIWWebSubMenu);

    //Part Name
    $SCondition = "  IdKey = '$objEditView->GroupIdKey' ";
    $Item = $objORM->Fetch($SCondition, 'Name,IdKey', TableIWWebMainMenu);
