...
$Enabled = true;
$strListHead = (new ListTools())->TableHead(array( FA_LC["main_name"], FA_LC["local_name"], FA_LC["branch"],FA_LC["weight"]), FA_LC["tools"]);
...
$strListBody = '';
foreach ($objORM->FetchAllWhitoutCondition('Name,LocalName,GroupIdKey,iw_product_weight_id,Enabled,id', TableIWWebSub4Menu) as $ListItem) {
    ...
    $ListItem->iw_product_weight_id =  '<input type="text" class="weight-sub4" maxlength="3" size="3" id="' . $ListItem->Name . '" value="' . @$objORM->Fetch($SCondition,'Weight',TableIWWebWeightPrice)->Weight . '">';
    ...
    $strListBody .= (new ListTools())->TableBody($ListItem, $ToolsIcons, 5, $objGlobalVar->en2Base64($ListItem->id . '::==::' . TableIWWebSub4Menu, 0));
}
