...
$Enabled = true;
$strListHead = (new ListTools())->TableHead(array(FA_LC["row"], FA_LC["name"], FA_LC["currency"]), FA_LC["tools"]);
...
$strListBody = '';
foreach ($objORM->FetchAllWhitoutCondition('IdKey,Name,CurrencyId,Enabled,IdRow', TableIWACountry) as $ListItem) {

    $SCondition = "IdKey = '$ListItem->CurrencyId'";
    ...
    $strListBody .= (new ListTools())->TableBody($ListItem, $ToolsIcons, 3, $objGlobalVar->en2Base64($ListItem->IdKey . '::==::' . TableIWWebSubMenu, 0));
}
