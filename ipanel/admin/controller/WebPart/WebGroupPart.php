...
$Enabled = true;
$strListHead = (new ListTools())->TableHead(array(FA_LC["row"], FA_LC["name"], FA_LC["part_name"]), FA_LC["tools"]);
...
foreach ($objORM->FetchAllWhitoutCondition('IdKey,Name,PartName,Enabled,IdRow', TableIWWebGroupPart) as $ListItem) {
    ...
    $strListBody .= (new ListTools())->TableBody($ListItem, $ToolsIcons, 3, $objGlobalVar->en2Base64($ListItem->IdKey . '::==::' . TableIWWebGroupPart, 0));
}
