...
$Enabled = true;
$strListHead = (new ListTools())->TableHead(array(FA_LC["row"],  FA_LC["group_name"], FA_LC["name"], FA_LC["trade_name"]), FA_LC["tools"]);
...
$strListBody = '';
foreach ($objORM->FetchAllWhitoutCondition('IdKey,GroupIdKey,Name,TradeName,Enabled,IdRow', TableIWTrade) as $ListItem) {
    ...
    $strListBody .= (new ListTools())->TableBody($ListItem, $ToolsIcons, 4, $objGlobalVar->en2Base64($ListItem->IdKey . '::==::' . TableIWTrade, 0));
}
