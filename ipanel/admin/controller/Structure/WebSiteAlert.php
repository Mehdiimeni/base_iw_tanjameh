<?php
///controller/Structure/WebSiteAlert.php
...
$Enabled = true;
$strListHead = (new ListTools())->TableHead(array(FA_LC["row"], FA_LC["name"], FA_LC["type"], FA_LC["content"]), FA_LC["tools"]);
...
$strListBody = '';
foreach ($objORM->FetchAllWhitoutCondition('IdKey,alert_name,alert_type,alert_content,Enabled,id', TableIWWebSiteAlert) as $ListItem) {
    ...
    $strListBody .= (new ListTools())->TableBody($ListItem, $ToolsIcons, 4, $objGlobalVar->en2Base64($ListItem->IdKey . '::==::' . TableIWWebSiteAlert, 0));
}