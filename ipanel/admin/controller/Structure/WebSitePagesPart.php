...
$Enabled = true;
$strListHead = (new ListTools())->TableHead(array(FA_LC["row"], FA_LC["local_name"], FA_LC["part"], FA_LC["website_page"]), FA_LC["tools"]);
...
foreach ($objORM->FetchAllWhitoutCondition('IdKey,title,name,iw_web_pages_id,Enabled,id', TableIWWebSitePagesPart) as $ListItem) {
    ...
    $strListBody .= (new ListTools())->TableBody($ListItem, $ToolsIcons, 4, $objGlobalVar->en2Base64($ListItem->IdKey . '::==::' . TableIWWebSitePagesPart, 0));
}