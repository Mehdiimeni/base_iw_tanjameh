<?php
///controller/Structure/WebSiteInfo.php
...
$Enabled = true;
$strListHead = (new ListTools())->TableHead(array(FA_LC["row"], FA_LC["name"], FA_LC["title"], FA_LC["email"], FA_LC["tel"]), FA_LC["tools"]);
...
$strListBody = '';
foreach ($objORM->FetchAllWhitoutCondition('IdKey,website_name,website_title,email,main_phone,Enabled,id', TableIWWebSiteInfo) as $ListItem) {
    ...
    $strListBody .= (new ListTools())->TableBody($ListItem, $ToolsIcons, 5, $objGlobalVar->en2Base64($ListItem->IdKey . '::==::' . TableIWWebSiteInfo, 0));
}