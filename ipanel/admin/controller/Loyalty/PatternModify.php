 ...
 */
        ...
        $Title = $objAclTools->CleanStr($objAclTools->JsonDecode($objAclTools->PostVarToJson())->Title);
        $FromCardNumber = (int)$objAclTools->strReplace($objAclTools->CleanStr($objAclTools->JsonDecode($objAclTools->PostVarToJson())->FromCardNumber), "-");
        $ToCardNumber = (int)$objAclTools->strReplace($objAclTools->CleanStr($objAclTools->JsonDecode($objAclTools->PostVarToJson())->ToCardNumber), "-");
        $arrPost = array('FromCardNumber' => $FromCardNumber, 'ToCardNumber' => $ToCardNumber, 'Title' => $Title);
...
if (@$objGlobalVar->RefFormGet()[0] != null) {
    ...
    if (isset($_POST['SubmitM'])) {
        ...
        if ($objAclTools->CheckNull($objAclTools->PostVarToJson())) {
            ...
            $FromCardNumber = (int)$objAclTools->strReplace($objAclTools->CleanStr($objAclTools->JsonDecode($objAclTools->PostVarToJson())->FromCardNumber), "-");
            $ToCardNumber = (int)$objAclTools->strReplace($objAclTools->CleanStr($objAclTools->JsonDecode($objAclTools->PostVarToJson())->ToCardNumber), "-");
            $arrPatch = array('FromCardNumber' => $FromCardNumber, 'ToCardNumber' => $ToCardNumber, 'Title' => $Title);
