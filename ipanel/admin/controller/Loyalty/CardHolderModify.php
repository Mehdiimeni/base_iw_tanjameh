<?php
//CardHolderModify.php

$apiMainName = 'CardHolder';
 ...
 */
        ...
        $FirstName = $objAclTools->CleanStr($objAclTools->JsonDecode($objAclTools->PostVarToJson())->FirstName);
        $LastName = $objAclTools->CleanStr($objAclTools->JsonDecode($objAclTools->PostVarToJson())->LastName);
        $arrPost = array('FirstName' => $FirstName, 'LastName' => $LastName, 'IsActive' => true);
...
if (@$objGlobalVar->RefFormGet()[0] != null) {
    ...
    if (isset($_POST['SubmitM'])) {
        ...
        if ($objAclTools->CheckNull($objAclTools->PostVarToJson())) {
            ...
            $FirstName = $objAclTools->CleanStr($objAclTools->JsonDecode($objAclTools->PostVarToJson())->FirstName);
            $LastName = $objAclTools->CleanStr($objAclTools->JsonDecode($objAclTools->PostVarToJson())->LastName);
            $arrPatch = array('FirstName' => $FirstName, 'LastName' => $LastName);
