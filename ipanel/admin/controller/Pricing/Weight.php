...
$Enabled = true;
$strListHead = (new ListTools())->TableHead(array(FA_LC["row"], FA_LC["weight"], FA_LC["normal"], FA_LC["extra"],  FA_LC['last_update']), FA_LC["tools"]);
...
$strListBody = '';
foreach ($objORM->FetchAllWhitoutCondition('IdKey,Weight,NormalPrice,ExtraPrice,ModifyDate,ModifyTime,ModifyId,Enabled,IdRow', TableIWWebWeightPrice) as $ListItem) {
    ...
    $strListBody .= (new ListTools())->TableBody($ListItem, $ToolsIcons, 5, $objGlobalVar->en2Base64($ListItem->IdKey . '::==::' . TableIWWebWeightPrice, 0));
}
