...
if (isset($_POST['SubmitM']) and @$objGlobalVar->RefFormGet()[0] == null) {
    ...
    if ($objAclTools->CheckNull($objAclTools->PostVarToJson())) {
        ...
        $TreatmentId = $objAclTools->CleanStr($objAclTools->JsonDecode($objAclTools->PostVarToJson())->TreatmentId);
        $Type = $objAclTools->CleanStr($objAclTools->JsonDecode($objAclTools->PostVarToJson())->Type);


        $arrPost = array('Customer_Id' => (int)$stdProfile->ApiId,'Operator' => 'Irancell' , 'TreatmentId' => $TreatmentId, 'Type' => "20000" , 'TreatmentAmount' => (int)$Type);
