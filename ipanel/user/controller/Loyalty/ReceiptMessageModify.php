<?php
//ReceiptMessageModify.php

$apiMainName = 'ReceiptMessage';
 ...
 */
        ...
        $Title = $objAclTools->CleanStr($objAclTools->JsonDecode($objAclTools->PostVarToJson())->Title);
        $Message = $objAclTools->CleanStr($objAclTools->JsonDecode($objAclTools->PostVarToJson())->Message);
        $arrPost = array('Title' => $Title, 'Message' => $Message, 'IsActive' => true);
...
if (@$objGlobalVar->RefFormGet()[0] != null) {
    ...
    if (isset($_POST['SubmitM'])) {
        ...
        if ($objAclTools->CheckNull($objAclTools->PostVarToJson())) {
            ...
            $Title = $objAclTools->CleanStr($objAclTools->JsonDecode($objAclTools->PostVarToJson())->Title);
            $Message = $objAclTools->CleanStr($objAclTools->JsonDecode($objAclTools->PostVarToJson())->Message);
            $arrPatch = array('Title' => $Title,'Message' => $Message);
