...
$Enabled = true;
$strListHead = (new ListTools())->TableHead(array(FA_LC["name"], FA_LC["message"], FA_LC["row"]), FA_LC["tools"]);
...
foreach ($objGlobalVar->ObjectJsonToSelectArray($objRNLS2->connect(), 1) as $ListItem) {
    ...
    $strListBody .= (new ListTools())->TableBody($ListItem, $ToolsIcons, 3, $objGlobalVar->en2Base64($ListItem->Id . '::==::' . $apiMainName, 0));
}
