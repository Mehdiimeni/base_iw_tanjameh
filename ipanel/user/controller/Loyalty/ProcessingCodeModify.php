<?php
//ProcessingCodeModify.php

$apiMainName = 'ProcessingCode';
 ...
 */
        ...
        $Title = $objAclTools->CleanStr($objAclTools->JsonDecode($objAclTools->PostVarToJson())->Title);
        $Code = (int)$objAclTools->CleanStr($objAclTools->JsonDecode($objAclTools->PostVarToJson())->Code);
        $arrPost = array('Title' => $Title,'Code' => $Code, 'IsActive' => true);
...
if (@$objGlobalVar->RefFormGet()[0] != null) {
    ...
    if (isset($_POST['SubmitM'])) {
        ...
        if ($objAclTools->CheckNull($objAclTools->PostVarToJson())) {
            ...
            $Title = $objAclTools->CleanStr($objAclTools->JsonDecode($objAclTools->PostVarToJson())->Title);
            $Code = (int)$objAclTools->CleanStr($objAclTools->JsonDecode($objAclTools->PostVarToJson())->Code);
            $arrPatch = array('Title' => $Title, 'Code' => $Code);
