...
if (isset($_POST['SubmitM']) and @$objGlobalVar->RefFormGet()[0] == null) {
    ...
    if ($objAclTools->CheckNull($objAclTools->PostVarToJson())) {
        ...
        $Amount = $objAclTools->CleanStr($objAclTools->JsonDecode($objAclTools->PostVarToJson())->Amount);
        $CellNumber = $objAclTools->CleanStr($objAclTools->JsonDecode($objAclTools->PostVarToJson())->CellNumber);
        $TopupType = $objAclTools->CleanStr($objAclTools->JsonDecode($objAclTools->PostVarToJson())->TopupType);


        $arrPost = array('Customer_Id' => (int)$stdProfile->ApiId, 'Operator' => $Operator, 'Amount' => (int)$Amount , 'CellNumber' => $CellNumber , 'Type' => $TopupType);
