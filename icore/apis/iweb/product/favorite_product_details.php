...
if (!empty($_POST['favorite_items'])) {

    $products_diteils = array();
    $currencies_conversion_id = trim($_POST['currencies_conversion_id']);
    foreach ($_POST['favorite_items'] as $key => $value) {

        $product_id = (int) $value;
        if ($objORM->DataExist("id = $product_id ", TableIWAPIProducts, 'id')) {
            $product = @$objORM->Fetch("id = $product_id", "id,Name,url_gender,url_category,url_group,Content,ImageSet,MainPrice,LastPrice,iw_api_product_type_id,iw_api_brands_id", TableIWAPIProducts);
