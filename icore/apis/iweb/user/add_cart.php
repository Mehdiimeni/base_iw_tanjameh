...
if (
    ...
    if (!$objORM->DataExist($SCondition, TableIWUserCartSet)) {

        $count_fav = $objORM->DataCount($SCondition, TableIWUserCartSet);

        if ($count_fav > 29) {
            $objORM->DeleteRow("user_id = $user_id or session_id = '$session_id' order by id limit 1", TableIWUserCartSet);
        }
        ...
        $objORM->DataAdd($InSet, TableIWUserCartSet);

    }

    $response = array(
        "success" => true,
        "item_count" => $objORM->DataCount("(user_id = $user_id or session_id = '$session_id')", TableIWUserCartSet)
    );
