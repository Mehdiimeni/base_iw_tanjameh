...
function get_search_info($search)
{
    $filds = array(
        'search' => $search
    );
    $objIAPI = set_server();
    return json_decode($objIAPI->GetPostApi('product/search', $filds));

}

function search_product_details($search, $page_condition)
{
    $objIAPI = set_server();
    ...
    return json_decode($objIAPI->GetPostApi('product/search_product_details', $filds));
}

function search_product_paging($limit, $total, $actual_link)
{
